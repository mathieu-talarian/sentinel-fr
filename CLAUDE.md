...
- `src/styles.css` is intentionally minimal — only universal-selector resets (`* { box-sizing }`), html/body height + zero margins, body font defaults, scrollbar pseudo-elements, the `:focus-visible` ring, and the `[data-density="compact"]` font tweak. Anything theme-token-bound belongs in StyleX, not here
...
- Shorthands StyleX rejects (silently — they compile but emit no rule): `background`, `border`, `borderTop/Bottom/Left/Right`, `outline`, multi-corner `borderRadius`. Decompose to longhand (`backgroundColor`, `borderTopColor`/`Style`/`Width`, `outlineStyle`/`Width`/`Color`, etc.). `flex` numeric values must be strings (`'1'`). When something visibly renders as the browser default (transparent div, `rgb(239,239,239)` button), suspect a dropped shorthand
